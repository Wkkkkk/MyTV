...
pub use channels::{
    admin_index, channel_create, channel_delete, channel_detail, channel_edit_form,
    channel_list, channel_new_form, channel_update,
};
pub use discover::{
    discover_add, discover_add_form, discover_m3u_search, discover_manual_resolve,
    discover_page, discover_youtube_search,
};
...
impl From<channel::Channel> for AdminChannelRow {
    fn from(ch: channel::Channel) -> Self {
        Self { id: ch.id, name: ch.name, category: ch.category, type_str: ch.r#type, sort_order: ch.sort_order }
    }
}

impl From<source::Source> for AdminSourceRow {
    fn from(s: source::Source) -> Self {
        Self { id: s.id, kind: s.kind, url: s.url, priority: s.priority, is_active: s.is_active }
    }
}

impl From<playlist_item::PlaylistItem> for AdminPlaylistItemRow {
    fn from(i: playlist_item::PlaylistItem) -> Self {
        Self { id: i.id, title: i.title, url: i.url, duration_secs: i.duration_secs, sort_order: i.sort_order }
    }
}
