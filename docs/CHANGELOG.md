...
done (2026-06-17): scoped to the low-risk, high-value step; the broad generic-CRUD collapse is deliberately **not** pursued. Shipped: the string-parameterized `model::update_health_sql` — which `format!`'d a table name into SQL (the *only* dynamic-table-name query in the codebase, shared across `sources` and `playlist_items`) — is retired; `source::update_health` and `playlist_item::update_health` now each inline a literal-table-name query, killing the silent field-skew hazard. **Reopened 2026-06-17 and re-closed after a function-by-function audit of `source.rs`/`playlist_item.rs`/`channel.rs`:** the "~2,000 lines of boilerplate" framing didn't hold — each file is ~150 lines of DB functions; the rest is non-duplicated domain logic (`is_observed_down`, `current_position`, `reap_stale_disabled`, anchor resolution, the `SourceKind`/`ChannelType` enums) plus tests. The only *truly* identical-modulo-table functions are `get` and `delete`. Everything else diverges on something real: `source.create` hard-codes `is_active=1`; `playlist_item` carries the `disabled_at` reap-clock CASE invariant in both `set_active` and `update_health`; `channel` has no `is_active`, no health columns, and a datetime `loop_anchor`. A generic `CrudModel<T>` trait or per-table macro would need so many knobs (insert-shape, optional health, the `disabled_at` CASE, per-table `ORDER BY`, channel's missing columns) that it would be larger and less navigable than the duplication it removes — a forced abstraction. The one defensible micro-move (a `get!`/`delete!` macro over the two identical functions) saves ~30 lines for one layer of indirection across the three hottest model files — judged a net loss for navigability. Decision: keep closed; the valuable risk-free part already shipped. Idea #50 from the 2026-06-16 architecture-deepening round.
